...
SizeHub is a PWA for wardrobe size management built with Next.js and Supabase. The project implements a measurement-first approach using the EN 13402 standard, with automatic brand size conversions, trust circles for sharing, Secret Giver (paid temporary access), and gift links.
...
- Planned tables in BUILD_PLAN.md: `brands`, `brand_size_maps`, `trust_circles`, `share_rules`, `shared_links`, `secret_giver_requests`, etc.
...
**Phase 1 (T0-T+4w)**: ZPR (measurements/labels/history), brand conversions
...
**Phase 4 (T+8-T+10w)**: Contextual notifications (CRON), weekly digest
